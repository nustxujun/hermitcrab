...
void AtmosphericScattering::initAtmospehre()
{
	...
	// rayleigh�㣬���������Ӳ�     //width,exp_term,exp_scale,linear_term,constant_term
	DensityProfileLayer rayleigh_layer = {0.0, 1.0 * use_rayleigh_scattering,
		-1.0 / kRayleighScaleHeight * use_rayleigh_scattering,
		0.0, 0.0};
	// mie�㣬���ܽ���            //width,exp_term,exp_scale,linear_term,constant_term
	...
	std::vector<double> solar_irradiance;       // ̫�����ն�
	std::vector<double> rayleigh_scattering;    // rayleighɢ��
	std::vector<double> mie_scattering;         // mieɢ��
	std::vector<double> mie_extinction;         // mie����
	std::vector<double> absorption_extinction;  // ���չ��ߵĿ�����������
	std::vector<double> ground_albedo;          // ���淴����
	...
	auto interpolate = [&](auto& function, double wavelength)
	{
		...
		for (unsigned int i = 0; i < wavelengths.size() - 1; ++i) {
			if (wavelength < wavelengths[i + 1]) {// ���Բ�ֵ
				double u =
